...
class GetChoreUSerService:
    ...
    def __user_family(self, chore_id: int, family_id: int) -> ChoreUserEntity:
        chore_user_entity = self.chore_repository.find_by_id_with_user(chore_id=chore_id, family_id=family_id)

        if chore_user_entity is None:
            raise NotFoundError(code=NotFoundErrorCodes.CHORE_NOT_FOUND.code())

        return chore_user_entity