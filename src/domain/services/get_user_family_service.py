...
class GetUserFamilyService:
    def __init__(self, user_repository: UserRepository):
        self.user_repository = user_repository

    @logging(show_args=True, show_return=True)
    def execute(self, user_id: int, family_id: int) -> UserFamilyEntity:
        try:
    ...
    def __user_family(self, user_id: int, family_id: int) -> UserFamilyEntity:
        user_family_entity = self.user_repository.find_by_id_with_family(user_id)

        if user_family_entity is None or user_family_entity.family.id != family_id:
            raise NotFoundError(code=NotFoundErrorCodes.USER_NOT_FOUND.code())

        return user_family_entity