...
from src.application.usecases.chores.assign_chore_to_me_use_case import (
    AssignChoreToMeUseCase,
)


class TestAssignChoreToMeUseCase(unittest.TestCase):
    def setUp(self):
        self.mock_repo = MagicMock()
        self.mock_get_chore = MagicMock()
        self.use_case = AssignChoreToMeUseCase(
    ...
    def test_execute_updates_chore_with_current_user_as_assignee(self):
        chore = ChoreEntity(
            chore_id=1,
            family_id=2,
            title="Tarefa",
        ...
        current_user = self._current_user(user_id=3, family_id=2)
        ...
        self.assertEqual(call_kwargs["update_chore_dto"].assigned_to_user_id, 3)
